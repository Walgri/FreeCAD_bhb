...
class FemInputWriterOOFEM(FemInputWriter.FemInputWriter):
    def __init__(
        ...
        # TODO should be in tasks module
            ...
            message = "Multiple Shellthicknesses for OOFEM not yet supported, the first thickness object is taken for all elements: {0}\n".format(sh_name)
    ...
    def write_OOFEM_input_file(self):
        ...
        '''
        # once we gone need the nodes, we may recode the mesh writer, because we would get the nodes twice
    ...
    def write_components_size_record(self, f):
        ...
        for femobj in allcss:  # femobj --> dict, FreeCAD document object is femobj['Object']
            cs_count += 1
        ...
        if len(self.material_objects) != 1:
            ...
            message = "Multiple Materials for OOFEM not yet supported, the first material is taken for all elements: {0}\n".format(mat_name)
            FreeCAD.Console.PrintError(message)
        ...
        for femobj in self.force_objects:  # femobj --> dict, FreeCAD document object is femobj['Object']
            if (femobj['RefShapeType'] == 'Edge') or (femobj['RefShapeType'] == 'Face'):
                FreeCAD.Console.PrintError('Constraint force on Edges and Faces will be ignored!\n')
                break
        tmp_list = [femobj for femobj in self.force_objects if femobj['RefShapeType'] == 'Vertex']
        ...
        # write values
        f.write('ndofman {0}  nelem {1}  ncrosssect {2}  nmat {3}  nbc {4}  nic {5}  nltf {6}  nset {7}\n'.format(
            nd_count, self.ele_count, self.cs_count, mat_count, bc_count, ic_count, tltf_count, set_count)
        )
    ...
    def write_cross_section_record(self, f):
        ...
        self.cs_set_number = 1  # we only have one cs set, late we gone write this inside the femobj (like we did with cs_number)
    ...
    def write_material_type_record(self, f):
        ''' *MaterialType (num#) (in) d # (rn)
                ...
                *EC2CreepMat n (rn) # [ begOfTimeOfInterest (rn) #] [ end-OfTimeOfInterest (rn) #] relMatAge (rn) # [ timeFactor (rn) #] stiffnessFactor (rn) #
                    [ tAlpha (rn) #] fcm28 (rn) # t0 (rn) # cem- Type (in) # [ henv (rn) #] h0 (rn) # shType (in) # [ spectrum ][ temperatureDependent ]
        '''
        if self.write_comments is True:
            f.write('#\n')
            f.write('#\n')
    ...
    def write_boundary_condition_record(self, f):
        ...
        for femobj in self.displacement_objects:  # femobj --> dict, FreeCAD document object is femobj['Object']
            ...
            if self.domain == '2dPlaneStress':
                ...
                if (disp_obj.xFix is True) and (disp_obj.xFree is False) and (disp_obj.xDisplacement == 0.0):
                    dofs_count += 1
                    dofs_numbers += '1'
                    val_value += ' 0'
                elif (disp_obj.xFix is False) and (disp_obj.xFree is False) and (disp_obj.xDisplacement != 0):
                    dofs_count += 1
                    dofs_numbers += ' 1'
                    val_value += ' {0}'.format(disp_obj.xDisplacement)
                if (disp_obj.yFix is True) and (disp_obj.yFree is False) and (disp_obj.yDisplacement == 0.0):
                    dofs_count += 1
                    dofs_numbers += ' 2'
                    val_value += ' 0'
                elif (disp_obj.yFix is False) and (disp_obj.yFree is False) and (disp_obj.yDisplacement != 0):
                    dofs_count += 1
                    dofs_numbers += ' 2'
                ...
                if (disp_obj.yFix is True) and (disp_obj.yFree is False) and (disp_obj.yDisplacement == 0.0):
                    ...
                    dofs_numbers += ' 2'
                    val_value += ' {0}'.format(disp_obj.yDisplacement)
                if (disp_obj.zFix is True) and (disp_obj.zFree is False) and (disp_obj.zDisplacement == 0.0):
                    dofs_count += 1
                    dofs_numbers += ' 3'
                    val_value += ' 0'
                elif (disp_obj.zFix is False) and (disp_obj.zFree is False) and (disp_obj.zDisplacement != 0):
                    dofs_count += 1
                    dofs_numbers += ' 3'
    ...
    def write_nodal_load_record(self, f):
        ...
        for femobj in self.force_objects:  # femobj --> dict, FreeCAD document object is femobj['Object']
            ...
            node_load = femobj['Object'].Force / len(femobj['Nodes'])  # this only work for loads on Vertexes!
            v1 = 0.0
            v2 = 0.0
            v3 = 0.0
    ...
    def write_set_record(self, f):
        ...
        # on mesh writing the elements are renumbered starting with 1, means the ele_count == max ele number
        ele_start = 1
