...
def setup_rcfibwall2d(doc=None, solver='ccxtools'):
    # setup reinfoced fib wall in 2D
    ...
    l7 = ln(v7, v8)
    l8 = ln(v8, v1)
    fibwall = doc.addObject("Part::Feature", "FIB_Wall")
    fibwall.Shape = Part.Face(Part.Wire([l1, l2, l3, l4, l5, l6, l7, l8]))
    ...
    if solver is None:
        ...
        solver = analysis.addObject(ObjectsFem.makeSolverCalculix(doc, 'SolverCalculiX'))[0]
        ...
        solver = analysis.addObject(ObjectsFem.makeSolverCalculixCcxTools(doc, 'CalculiXccxTools'))[0]
        solver.AnalysisType = 'static'
        solver.GeometricalNonlinearity = 'linear'
        solver.ThermoMechSteadyState = False
        solver.MatrixSolverType = 'default'
    ...
    # shell thickness
    thickness = analysis.addObject(ObjectsFem.makeElementGeometry2D(doc, 0, 'ShellThickness'))[0]
    thickness.Thickness = 150.0

    # material
    matrixprop = {}
    matrixprop['Name'] = "FibConcrete"
    ...
    matrixprop['Density'] = '2400 kg/m^3'
    reinfoprop = {}
    reinfoprop['Name'] = "FibReinforcement"
    reinfoprop['YieldStrength'] = "315 MPa"
    reinfoprop['ReinforcementRatio'] = "0.0"  # not an official FreeCAD material property
    material_reinforced = analysis.addObject(ObjectsFem.makeMaterialReinforced(doc, 'FemMaterialReinforced'))[0]
    ...
    fixed_constraint.References = [(fibwall, "Edge1"), (fibwall, "Edge5")]

    # force constraint
    force_constraint = doc.Analysis.addObject(ObjectsFem.makeConstraintForce(doc, name="ConstraintForce"))[0]
    force_constraint.References = [(fibwall, "Edge7")]
    force_constraint.Force = 1000000.0
    force_constraint.Direction = (fibwall, ["Edge8"])
    ...
    displacement_constraint.References = [(fibwall, "Face1")]
    displacement_constraint.zFix = True

    # mesh
    from femexamples.meshes.mesh_fib_wall_2d_tria6 import create_nodes, create_elements
...
'''
from femexamples import rc_fib_wall_2d as fib2d
fib2d.setup_rcfibwall2d()
